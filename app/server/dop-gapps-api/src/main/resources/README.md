...
# Java JAR
java -jar dop-gapps-server.jar --spring.profiles.active=local
```

**H2 콘솔 접속**:
- URL: http://localhost:8080/h2-console
- JDBC URL: `jdbc:h2:mem:gapps`
- Username: `sa`
- Password: (비어있음)
...
export DB_PORT=5432
export DB_NAME=gapps
export DB_USERNAME=gapps
...
**또는 인라인 설정**:
```bash
./gradlew bootRun --args='--spring.profiles.active=dev --DB_HOST=localhost --DB_USERNAME=gapps --DB_PASSWORD=dev123'
```
...
export DB_PORT=5432
export DB_NAME=gapps
export DB_USERNAME=gapps_prod
export DB_PASSWORD=secure-password
export ENCRYPTION_KEY=your-base64-encoded-256-bit-key

# 실행
java -jar dop-gapps-server.jar --spring.profiles.active=prod
```
...
| `DB_NAME` | gapps | 데이터베이스명 |
| `DB_USERNAME` | gapps (dev) / 필수 (prod) | DB 사용자명 |
...
```bash
# 특정 패키지 로그 레벨 변경
./gradlew bootRun --args='--spring.profiles.active=local --logging.level.com.daou.dop.gapps=TRACE'
...
- 프로파일이 `local`인지 확인
- URL: `http://localhost:8080/h2-console`
- JDBC URL: `jdbc:h2:mem:gapps` (정확히 입력)
...
3. **Environment variables** (Dev/Prod):
   ```
   DB_HOST=localhost;DB_USERNAME=gapps;DB_PASSWORD=dev123
   ```
  ...
  "configurations": [
    {
      ...
      "request": "launch",
      "mainClass": "com.daou.dop.gapps.server.GlobalAppsServerApplication",
